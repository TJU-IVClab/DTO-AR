...
namespace ORB_SLAM3
{
	...
	Sophus::SE3f Tracking::GrabImageRGBD(FrameData* mpFrameData, string filename) // !IVC-lab@lee
	{
		...
		RuntimeProfile::Scope preparation(RuntimeProfile::Preparation);
#endif
		// Input is read synchronously before the next acquisition; keyframes keep copies.
		mImGray = mpFrameData->t_u8;
		cv::Mat imDepth;
		const cv::Mat& imNorm = mpFrameData->n_f32;
		const cv::Mat& imConf = mpFrameData->c_f32;
		double timestamp = mpFrameData->frameTimestamp;

		if ((fabs(mDepthMapFactor - 1.0f) > 1e-5) || mpFrameData->d_f32.type() != CV_32F)
			mpFrameData->d_f32.convertTo(imDepth, CV_32F, mDepthMapFactor);
		else
			imDepth = mpFrameData->d_f32.clone();
...
#if ORB_SLAM3_RUNTIME_PROFILE
		RuntimeProfile::Scope mapWait(RuntimeProfile::MapWait);
#endif
	...
	void Tracking::TrackMS() // !IVC-lab@lee
	{
		...
		// Get Map Mutex -> Map cannot be changed
		// Measure acquisition only; the protected work has its own categories.
#if ORB_SLAM3_RUNTIME_PROFILE
		RuntimeProfile::Scope mapWait(RuntimeProfile::MapWait);
#endif
		unique_lock<mutex> lock(pCurrentMap->mMutexMapUpdate);
	...
	void Tracking::MSInitialization() {
		if (mCurrentFrame.N > 5)
		{
			...
			if (!mpFD->d_f32.empty() && !mpFD->t_u8.empty())
			{
				...
				mCurrentFrame.CorrDepthInEllipses(mpFD->d_f32, depth_mt);
#else
				mpFD->d_f32.copyTo(depth_mt);
#endif
	...
	{
#if ORB_SLAM3_RUNTIME_PROFILE
		RuntimeProfile::Scope refinement(RuntimeProfile::Pose);
#endif
	...
	{
#if ORB_SLAM3_RUNTIME_PROFILE
		RuntimeProfile::Scope keyframe(RuntimeProfile::Keyframe);
#endif
