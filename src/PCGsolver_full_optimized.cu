...
__forceinline__ __device__ float subsample_bilinear_strict0(
    const float* __restrict__ img, const float2& u_dist, int width, int height)
{
...
__global__ void buildEdgeNormalEquations(
    ...
    float* __restrict__ residual_sum_sq,      // optional (global) sum of w*e_geo^2
    unsigned int* __restrict__ residual_count // optional (global) count of valid residuals
    )
{
    // IMPORTANT:
...
#pragma unroll
    ...
    if (in) {
        ...
        bool ok = computeGeoResidualAndJrel(
            idx, edge.depth_i, edge.normal_i, edge.depth_j,
            cam, raymap, edge.T_ij,
            e_geo, J_rel);

        if (ok) {
            // Jj = J_rel * AdInv_i (same as original code indexing)
            float Jj[6];
...
#pragma unroll
            for (int r = 0; r < 6; ++r) {
                float tmp = 0.f;
#pragma unroll
                for (int c = 0; c < 6; ++c) tmp = fmaf(J_rel[c], A[r * 6 + c], tmp);
                Jj[r] = tmp;
            }
...
#pragma unroll
            for (int k = 0; k < 6; ++k) g[k] = w * e_geo * Jj[k];

            // Monitoring: weighted squared residual and valid count
            res2 = w * e_geo * e_geo;
            cnt = 1u;
...
#pragma unroll
                for (int b = a; b < 6; ++b) {
                    int id = sym6_idx(a, b);
                    A21[id] = fmaf(w * Jj[a], Jj[b], A21[id]);
                }
            }
        }
    }
    ...
    //   - 28 floats: (6 g + 21 A + 1 residual sumsq)
    //   - 1 uint32 : count
    int tid = (int)threadIdx.y * kBlockX + (int)threadIdx.x;
    int lane = tid & 31;
    int warp = tid >> 5;

    __shared__ float shf[kWarpsPerBlock][28];
    __shared__ unsigned int shc[kWarpsPerBlock];
...
#pragma unroll
    ...
    {
        unsigned int c = warpReduceSumUInt(cnt);
        if (lane == 0) shc[warp] = c;
    }
    __syncthreads();
...
#pragma unroll
        ...
        unsigned int c = (lane < kWarpsPerBlock) ? shc[lane] : 0u;
        c = warpReduceSumUInt(c);
        if (lane == 0) shc[0] = c;
    }
    __syncthreads();
...
#pragma unroll
        ...
        if (residual_sum_sq) atomicAdd(residual_sum_sq, shf[0][27]);
        if (residual_count) atomicAdd(residual_count, shc[0]);
    }
}
    ...
    float tol)
{
    ...
    float* d_geo_sumsq = nullptr;           // sum of w*e_geo^2
    unsigned int* d_geo_count = nullptr;    // count of valid residuals
