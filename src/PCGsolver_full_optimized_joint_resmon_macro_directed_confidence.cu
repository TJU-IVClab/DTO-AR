	...
	const std::string& png_path = "relative_pose_confidence_matrix.png")
{
	...
	for (int i = 0; i < numFrames; ++i) {
		conf.at<float>(i, i) = 1.0f;
	}

	std::ofstream ofs(dbPrefix + "/" + csv_path);
	...
	cv::imwrite(dbPrefix + "/" + png_path, visColor);
}
	...
	float tol)
{
	...
	for (int iter = 0; iter < maxIter; ++iter)
	{
		...
		if (shouldStop) {
			...
			std::vector<float> edge_energy_ms;
			edge_energy_ms.reserve(numEdges);
			std::vector<float> edge_conf(numEdges, 0.0f);

			const float N0 = 0.02f * float(width * height);

			for (int e = 0; e < numEdges; ++e) {
				const unsigned int Ng = h_edge_geo_count[e];
				const unsigned int Np = h_edge_pho_count[e];
				const unsigned int N = Ng + Np;
				if (N == 0u) continue;
				...
				const float Ebar = E / float(N); // weighted mean squared residual
				edge_conf[e] = Ebar;             // temporary storage before [0,1] mapping
				edge_energy_ms.push_back(Ebar);
			}

			const float tau = robustMedianPositive(edge_energy_ms);

			for (int e = 0; e < numEdges; ++e) {
				const unsigned int N = h_edge_geo_count[e] + h_edge_pho_count[e];
				if (N == 0u) {
					edge_conf[e] = 0.0f;
					continue;
				}

				const float Ebar = edge_conf[e];
				const float c_res = 1.0f / (1.0f + Ebar / (tau + 1e-12f));
				const float c_sup = 1.0f - std::exp(-float(N) / (N0 + 1e-12f));
				edge_conf[e] = clamp01f(c_res * c_sup);
			}

			saveRelativePoseConfidenceMatrix(
				h_edges, edge_conf, numFrames,
				"relative_pose_confidence_matrix.csv",
				"relative_pose_confidence_matrix.png");

			cout << "[Dense BA] saved directed relative pose confidence matrix: "
				<< "relative_pose_confidence_matrix.csv / .png" << endl;
#endif
