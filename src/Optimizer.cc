...
namespace {
class WsbaIterationProbe {
    using Clock = ORB_SLAM3::RuntimeProfile::Clock;
    using Record = ORB_SLAM3::RuntimeProfile::DetailRecord;
    struct Action : g2o::HyperGraphAction {
        Record& record;
        Clock::time_point& start;
        bool before;
        Action(Record& r, Clock::time_point& t, bool pre)
            : record(r), start(t), before(pre) {}
        g2o::HyperGraphAction* operator()(const g2o::HyperGraph*, Parameters* parameters) override {
            const auto now = Clock::now();
            if (before) start = now;
            else {
                const auto* iteration = static_cast<ParametersIteration*>(parameters);
                record.iterationIds.push_back(iteration->iteration);
                record.iterationMs.push_back(ORB_SLAM3::RuntimeProfile::Milliseconds(start, now));
            }
            return this;
        }
    };
    ...
    Action pre, post;
public:
    WsbaIterationProbe(g2o::SparseOptimizer& opt, Record& record)
        : optimizer(opt), pre(record, start, true), post(record, start, false) {
        record.iterationIds.reserve(10);
        record.iterationMs.reserve(10);
        optimizer.addPreIterationAction(&pre);
        optimizer.addPostIterationAction(&post);
    }
    ~WsbaIterationProbe() {
        optimizer.removePreIterationAction(&pre);
        optimizer.removePostIterationAction(&post);
    }
    WsbaIterationProbe(const WsbaIterationProbe&) = delete;
    WsbaIterationProbe& operator=(const WsbaIterationProbe&) = delete;
};
}
#endif
...
namespace ORB_SLAM3
{
	...
	{
#if ORB_SLAM3_RUNTIME_PROFILE && ORB_SLAM3_RUNTIME_PROFILE_DETAIL
		RuntimeProfile::DetailScope detail(true);
		detail.record.keyframe = pKF->mnId;
#endif
		// Local KeyFrames: First Breath Search from Current Keyframe
		list<KeyFrame*> lLocalKeyFrames;
		...
		num_fixedKF = lFixedCameras.size() + num_fixedKF;
#if ORB_SLAM3_RUNTIME_PROFILE && ORB_SLAM3_RUNTIME_PROFILE_DETAIL
		detail.record.localKF = static_cast<int>(lLocalKeyFrames.size());
		detail.record.fixedKF = num_fixedKF;
		detail.record.points = num_MPs;
#endif
		...
		num_edges = nEdges;
#if ORB_SLAM3_RUNTIME_PROFILE && ORB_SLAM3_RUNTIME_PROFILE_DETAIL
		detail.record.edges = num_edges;
		detail.record.graphReady = true;
#endif
...
#if ORB_SLAM3_RUNTIME_PROFILE && ORB_SLAM3_RUNTIME_PROFILE_DETAIL
		WsbaIterationProbe iterationProbe(optimizer, detail.record);
		detail.Next(3);
#endif
		const int wsbaOptimizeReturn = optimizer.optimize(5);
#if ORB_SLAM3_RUNTIME_PROFILE && ORB_SLAM3_RUNTIME_PROFILE_DETAIL
		detail.record.optimizeReturn = wsbaOptimizeReturn;
#endif
#if ORB_SLAM3_RUNTIME_PROFILE && ORB_SLAM3_RUNTIME_PROFILE_DETAIL
		detail.Next(4);
#endif
#if !ORB_SLAM3_WSBA_INITIAL_CHI
		// A stop before the first iteration leaves edge errors unevaluated.
		if (wsbaOptimizeReturn <= 0)
