...
#pragma unroll
    ...
    // Block-level reduction for 27 floats (6 g + 21 A)
    int tid = (int)threadIdx.y * kBlockX + (int)threadIdx.x;
    int lane = tid & 31;
    int warp = tid >> 5;

    __shared__ float sh[kWarpsPerBlock][27];
...
#pragma unroll
        for (int i0 = 0; i0 < 27; ++i0) {
            float s = (lane < kWarpsPerBlock) ? sh[lane][i0] : 0.f;
            s = warpReduceSum(s);
            if (lane == 0) sh[0][i0] = s;
        }
    }
...
#pragma unroll
        for (int k = 0; k < 6; ++k) atomicAdd(eg + k, sh[0][k]);
#pragma unroll
        for (int k = 0; k < 21; ++k) atomicAdd(eA + k, sh[0][6 + k]);
    }
}
    ...
    float tol)
{
    ...
    for (int iter = 0; iter < maxIter; ++iter)
    {
        ...
        cout << "[Dense BA] iter " << iter << " stepNorm=" << stepNorm << " time=" << t_opt << endl;
