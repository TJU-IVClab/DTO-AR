...
namespace ORB_SLAM3 {
	...
	Settings::Settings(const std::string& configFile, const int& sensor) :
		...
		if (sensor_ == System::RGBD || sensor_ == System::IMU_RGBD) {
			bNeedToUndistort_ = true; // !IVC-lab@lee: forced undistortion
			readRGBD(fSettings);
	...
	void Settings::readRGBD(cv::FileStorage& fSettings) {
		...
		bf_ = b_ * calibration1_->getParameter(0);
		// !IVC-lab@lee
		LEDPower_ = readParameter<double>(fSettings, "Photoneo.LEDPower", found);
		Confidence_ = readParameter<double>(fSettings, "Photoneo.Confidence", found);
	}
