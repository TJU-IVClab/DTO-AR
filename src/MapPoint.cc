...
namespace ORB_SLAM3
{
	...
	void MapPoint::ComputeDistinctiveDescriptors(cv::Mat& desc)
	{
		{
	...
	void MapPoint::UpdateNormalAndDepth()
	{
		...
		if (mDescriptor.cols == 132) // !IVC-lab@lee, useMatteSpots
		{
			for (map<KeyFrame*, tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
			{
				KeyFrame* pKF = mit->first;
				...
				if (leftIndex != -1) {
					...
					pKF->UnprojectRGBD(leftIndex, x3D, n3D);
					normal += n3D;
					n++;
				}
			}
...

			{
				...
				mfMaxDistance = dist * 1.0f;
				mfMinDistance = mfMaxDistance / 1.0f;
				mNormalVector = normal / normal.norm();
				/*mDescriptor.at<float>(1) = mNormalVector.x();
				mDescriptor.at<float>(2) = mNormalVector.y();
				mDescriptor.at<float>(3) = mNormalVector.z();*/
			}
		}
		else {
			for (map<KeyFrame*, tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
