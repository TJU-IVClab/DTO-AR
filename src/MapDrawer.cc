...
namespace ORB_SLAM3
{
	...
	void MapDrawer::DrawMapPoints()
	{
		...
		for (size_t i = 0; i < vpMPs.size(); i++)
		{
			...
			Eigen::Matrix<float, 3, 1> pos = vpMPs[i]->GetWorldPos();
			glColor3f(1.0, 0.0, 0.0);
			glRasterPos3f(pos(0), pos(1), pos(2)); // 设置文本位置
			glScalef(0.002f, 0.002f, 0.002f);  // 调整文字大小
			string ID = to_string(vpMPs[i]->GetDescriptor().at<float>(0));
			cout << "AAAA: " << ID << endl;
			pangolin::GlFont::I().Text("%d", ID).Draw();
	...
	void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph, const bool bDrawInertialGraph, const bool bDrawOptLba)
	{
		...
		if (bDrawGraph)
		{
			...
			{
				// Covisibility Graph
				const vector<KeyFrame*> vCovKFs = vpKFs[i]->GetCovisiblesByWeight(vpKFs[i]->N > 50 ? 100 : 10); // !IVC-lab@lee
