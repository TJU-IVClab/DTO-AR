...
namespace ORB_SLAM3
{
	...
	cv::Mat FrameDrawer::DrawFrame(float imageScale)
	{
		...
		else if (state == Tracking::OK) //TRACKING
		{
			...
			for (int i = 0; i < n; i++)
			{
				if (currentFrame.mergedCenters.size()) // !IVC-lab@lee
				{
					cv::RotatedRect& cmel = currentFrame.mergedCenters[i].ellipse;
					cv::ellipse(im, cmel, standardColor, cv::FILLED, cv::LINE_AA);
					/*vector<cv::Point2f> pts;
					cmel.points(pts);
					for (cv::Point2f& pt : pts) {
						cv::circle(im, pt, 2, cv::Scalar(255, 255, 0), 1, cv::LINE_AA);
					}*/
					if (vbMap[i])
					{
						...
						float ID = currentFrame.mDescriptors.row(i).at<float>(0);
						cv::Point2f textPos = point + cv::Point2f(10, 10);
						cv::putText(im, to_string(static_cast<int>(ID)), textPos, cv::FONT_HERSHEY_TRIPLEX, 0.75, cv::Scalar(0, 0, 255), 2);
						mnTracked++;
					}
				}
				else {
