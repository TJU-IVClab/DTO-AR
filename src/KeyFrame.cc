...
namespace ORB_SLAM3
{
		...
		mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
	{
		...
		// !IVC-lab@lee
		hasValidMSTransform = F.hasValidMSTransform;
	}
	...
	{
		unique_lock<mutex> lock(mMutexConnections);
		//cout << "Keyframe: " << frameIndex << " with " << (int)mvpOrderedConnectedKeyFrames.size() << " ConnectedKeyFrames." << endl;
	...
	bool KeyFrame::UnprojectRGBD(int i, Eigen::Vector3f& x3D, Eigen::Vector3f& n3D)
	{
		const float z = mvDepth[i];
		...
		const float nx = mDescriptors.at<float>(i, 1);
		const float ny = mDescriptors.at<float>(i, 2);
		const float nz = mDescriptors.at<float>(i, 3);
		Eigen::Vector3f n3Dc(nx, ny, nz);
