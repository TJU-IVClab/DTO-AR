...
#pragma unroll
    ...
    // @HESAI: 
    // CUDA 里`__shfl_down_sync`只能在同一个`warp`内通信。
    // 于是，让第0个wrap的lane读shared memory(0-7读shf,8-31还是0)，然后复用wrap reduction完成最后一步求和。
    ...
    float tol)
{
    ...
    for (int i = 0; i < numFrames; ++i)
    {
        ...
        std::vector<ORB_SLAM3::KeyFrame*> vCov = pKF->GetBestCovisibilityKeyFrames(8);
    ...
    for (int iter = 0; iter < maxIter; ++iter)
    {
        ...
        if (shouldStop) {
            ...
            // Reset global accumulators for the final post-update evaluation.
            CUDA_CHECK(cudaMemsetAsync(d_geo_sumsq, 0, sizeof(float), stream));
            CUDA_CHECK(cudaMemsetAsync(d_geo_count, 0, sizeof(unsigned int), stream));
            CUDA_CHECK(cudaMemsetAsync(d_pho_sumsq, 0, sizeof(float), stream));
            CUDA_CHECK(cudaMemsetAsync(d_pho_count, 0, sizeof(unsigned int), stream));
            ...
            buildEdgeNormalEquations << <grid2d, block2d, 0, stream >> > (
                ...
                , d_geo_sumsq, d_geo_count, d_pho_sumsq, d_pho_count
#if DENSEBA_ENABLE_GEO_VIS || DENSEBA_ENABLE_PHO_VIS
                ,
#if DENSEBA_ENABLE_GEO_VIS
                nullptr,
            ...
            CUDA_CHECK(cudaMemcpyAsync(h_edge_pho_count.data(), d_edge_pho_count,
                numEdges * sizeof(unsigned int), cudaMemcpyDeviceToHost, stream));
            // Read the newly evaluated global statistics before printing final.
            CUDA_CHECK(cudaMemcpyAsync(&h_geo_sumsq, d_geo_sumsq,
                sizeof(float), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaMemcpyAsync(&h_geo_count, d_geo_count,
                sizeof(unsigned int), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaMemcpyAsync(&h_pho_sumsq, d_pho_sumsq,
                sizeof(float), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaMemcpyAsync(&h_pho_count, d_pho_count,
                sizeof(unsigned int), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaStreamSynchronize(stream));

            // State iter+1 follows the last applied update. No extra solve/update.
            // last_stepNorm is the same increment already reported by iter.
            const double final_geo_rms = h_geo_count > 0u
                ? std::sqrt(double(h_geo_sumsq) / double(h_geo_count)) : 0.0;
            const double final_pho_rms = h_pho_count > 0u
                ? std::sqrt(double(h_pho_sumsq) / double(h_pho_count)) : 0.0;
            const double final_E_geo = double(kWGeo) * double(h_geo_sumsq);
            const double final_E_pho = double(kWPho) * double(h_pho_sumsq)
                * double(kPhoGain_exp_dec) * double(kPhoGain_exp_dec);

            cout << "[Dense BA] final state=" << iter + 1
                << " last_stepNorm=" << stepNorm
                << " geo_rms=" << final_geo_rms / kDepthScale << " /mm"
                << " geo_cnt=" << h_geo_count
                << " pho_rms=" << final_pho_rms * 255.0 << " /255"
                << " pho_cnt=" << h_pho_count
                << " stop_reason=" << ((float)stepNorm < tol ? "step_tol" : "max_iter")
                << endl;
            cout << "E_geo=" << final_E_geo
                << " E_pho=" << final_E_pho
                << " E_total=" << final_E_geo + final_E_pho << endl;
